...
class MNISTDiffusion(nn.Module):
    ...
    @torch.no_grad()
    def reconstruct(self, x, noise, device="cuda"):
        """
        Evaluates how well the model predicts the noise by adding noise to an original image.

        Args:
            x (Tensor): Imágenes originales, rango [-1, 1], shape (B,C,H,W)
            noise (Tensor): Ruido gaussiano de mismo shape que x
            device (str): Dispositivo de cómputo

        Returns:
            Tensor: Reconstrucciones x0_pred en rango [-1, 1]
        """
        t=torch.randint(0,self.timesteps,(x.shape[0],)).to(x.device)
        x_t=self._forward_diffusion(x,t,noise)
        ...
        alpha_cumprod_t = self.alphas_cumprod.gather(-1, t).reshape(x_t.shape[0], 1, 1, 1)

        x0_pred = (
            x_t - torch.sqrt(1 - alpha_cumprod_t) * pred_noise
